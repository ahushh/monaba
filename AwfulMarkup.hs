  ...
  case maybePost of
    Just post -> do
      let linkType       = if B.toString linkType' == "&gt;&gt;" then ">>" else "##"
          localBoard thr = B.concat ["<a onclick='highlightPost(",postId,")' href='/thread/",board,"/",thr,"#",postId,"'>",linkType,postId,"</a>"]
          crossBoard thr = B.concat ["<a onclick='highlightPost(",postId,")' href='/thread/",board,"/",thr,"#",postId,"'>",linkType,"/",board,"/",postId,"</a>"]
