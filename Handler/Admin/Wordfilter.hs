...
form :: Html -> MForm Handler (FormResult (Maybe Text, WordfilterDataType, Textarea, [WordfilterAction], Textarea) , Widget)
form extra = do
  ...
  let types :: [(Text, WordfilterDataType)]
      ...
      actions = map (first msgrender) [(MsgWordfilterBan, WordfilterBan), (MsgWordfilterHB, WordfilterHB), (MsgWordfilterHBHide, WordfilterHBHide), (MsgWordfilterDeny, WordfilterDeny)]
  ...
  let result = (,,,,) <$> boardRes <*> typeRes <*> dataRes <*> actionRes <*> msgRes
...
postAdminWordfilterR = do
  ...
  case result of
   ...
   FormSuccess (boardVal, typeVal, dataVal, actionVal, msg) -> do
