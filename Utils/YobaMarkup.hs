...
processMarkup xs board thread = Textarea <$> foldM f "" xs
  where
    ...
    refHtml acc brd "0" p ref pId = [st|#{acc}<a data-post-id=#{pId} data-post-local-id=#{p} data-board=#{brd} data-thread-local-id=0 onmouseover="timeout(this, function(){showPopupPost(event,#{pId},#{p},'#{brd}')},700)" onclick='highlightPost("p#{pId}")' href='/#{brd}/#{p}'>#{ref}</a> |]
    refHtml acc brd thr p ref pId = [st|#{acc}<a data-post-id=#{pId} data-post-local-id=#{p} data-board=#{brd} data-thread-local-id=#{thr} onmouseover="timeout(this, function(){showPopupPost(event,#{pId},#{p},'#{brd}')},700)" onclick='highlightPost("p#{pId}")' href='/#{brd}/#{thr}##{p}'>#{ref}</a> |]
